...
export default function AdminJudgeSchedulingPage() {
  ...
  async function fetchBulkNotifyMeta() {
    try {
      const res = await fetch("/api/admin/data/semifinals-judges/notify", { credentials: "include" })
      if (res.ok) {
        const data = await res.json()
  ...
  async function handleBulkNotify() {
    const judgesWithEmail = judges.filter((j) => j.email)
    if (judgesWithEmail.length === 0) {
      showToast("No judges with email addresses to notify", "error")
      return
    }
    if (!confirm(`Send invitation emails to ${judgesWithEmail.length} judge${judgesWithEmail.length !== 1 ? "s" : ""}? This will direct them to the scheduling page.`)) return

    setIsBulkSending(true)
    try {
      const res = await fetch("/api/admin/data/semifinals-judges/notify", {
        method: "POST",
        credentials: "include",
      })
      const data = await res.json()
      ...
      showToast(`Invitation sent to ${data.sent} judge${data.sent !== 1 ? "s" : ""}${data.failed ? ` (${data.failed} failed)` : ""}`)
  ...
  useEffect(() => { fetchJudges(); fetchBulkNotifyMeta() }, [])
    ...
    <div className="p-8 lg:p-12">
      ...
      <div className="mb-10 bg-neutral-50 border border-neutral-200 rounded-md p-5">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h3 className="text-sm font-bold text-black leading-5">Notify All Judges</h3>
            <p className="text-[13px] text-neutral-500 mt-1 leading-5">
              Send an invitation email to all judges directing them to schedule their judging session.
            </p>
          ...
          <button
            ...
            disabled={isBulkSending || judges.length === 0}
