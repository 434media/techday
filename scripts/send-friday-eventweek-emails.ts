// Send Friday "Event Week Is Here" email to ALL active registrants
// Determines event type per registrant and sends the correct variant
// Run with: npx tsx scripts/send-friday-eventweek-emails.ts
...
async function main() {
  console.log("Sending Friday Event Week emails to all active registrants...\n")
  ...
  for (let i = 0; i < registrants.length; i += BATCH_SIZE) {
    ...
    for (const reg of batch) {
      ...
      const content = getFridayContent(reg.firstName, eventType)
      ...
      try {
        ...
        if (result.error) {
          ...
          console.log(`  OK: ${reg.email} (${eventType}) — ${result.data?.id}`)
          sent++
        }
      } catch (err) {
