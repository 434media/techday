// Preview finalist notification email — sends to jesse@434media.com only
// Run with: npx tsx scripts/send-finalist-preview.ts
...
function getFinalistEmailContent(founderName: string, companyName: string) {
...
async function main() {
  console.log("Sending finalist preview email to", PREVIEW_EMAIL)

  // Use sample data for preview
  const founderName = "Founder"
  const companyName = "Sample Company"

  const content = getFinalistEmailContent(founderName, companyName)
  ...
  try {
    ...
    console.log("Preview sent successfully!", result.data)
  } catch (error) {
